...
export const CAPSTONE_SPECS: Record<string, BriefSpec> = {
  ...
  "gis-and-drone-mapping": {
    ...
    tasks: [
      ...
      "**Remote sensing analysis.** Use satellite imagery to calculate at least one **spectral index** (for example NDVI) and, if useful, a **change detection** between two dates (Day 4). State limitations.",
      "**Drone survey component.** Either **(a)** plan and fly a small survey yourself, following the regulations in Day 5, or **(b)** if you cannot fly, use **open drone imagery** (for example from OpenAerialMap) for your area, *or* write a **complete drone survey specification** (Day 6) for the field you would survey. Say clearly which you did. Include the GSD and flight-height calculation.",
      ...
      "**Thematic maps.** Produce **at least five well-designed maps** (Day 7) that build the story from inputs to final suitability zones.",
