...
import { fetchUngradedSubmissions } from "./assignmentsSlice";

export function InstructorGradingQueuePage() {
  const dispatch = useAppDispatch();
  const { ungradedSubmissions, status, error } = useAppSelector((state) => state.assignments);
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-gray-900">Grading queue</h1>
      ...
      {status === "succeeded" && ungradedSubmissions.length === 0 && (
        <div className="mt-8 rounded-lg border border-dashed border-gray-300 p-8 text-center">
          <p className="text-gray-600">No submissions waiting for grading.</p>
        </div>
      )}
